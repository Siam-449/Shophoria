...
export async function getTimeOffers() {
  try {
    ...
    const q = query(timeOffersCol, where("expiresAt", ">", now));
    ...
    const offersWithProducts = await Promise.all(offerSnapshot.docs.map(async (offerDoc) => {
      const offerData = offerDoc.data();
      const product = await getProduct(offerData.productId);

      if (product) {
        return {
          ...product, // All base product data (id, name, image, desc, etc.)
          price: offerData.discountPrice,
          originalPrice: offerData.originalPrice,
          expiresAt: offerData.expiresAt.toDate(), // Convert Firestore Timestamp to JS Date
        };
      }
      return null;
    }));
    
    // Filter out any nulls if a product wasn't found
