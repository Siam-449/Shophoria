...
export async function getTimeOffers() {
  try {
    ...
    const offersWithProducts = await Promise.all(activeOfferDocs.map(async (offerDoc) => {
      ...
      if (!productId) {
        console.warn(`Offer document ${offerDoc.id} is missing a productId.`);
        return null;
      }
      ...
      if (product) {
        ...
        // Coerce string price to number if necessary
        ...
        if (typeof discountPrice === 'number' && discountPrice > 0) {
            return {
             ...
             console.warn(`Offer for product ${productId} has an invalid or missing timediscountPrice. Value:`, offerData.timediscountPrice);
             return null;
        }
      }
