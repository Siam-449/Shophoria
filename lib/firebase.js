...
export async function getTimeOffers() {
  try {
    ...
    const offersWithProducts = await Promise.all(activeOfferDocs.map(async (offerDoc) => {
      ...
      // Fetch the full product details using the productId from the offer
      const product = await getProduct(offerData.productId);

      if (product) {
        // Merge offer data (like special price) with product data
        return {
          ...product,
          price: offerData.timediscountPrice,
    ...
    // Filter out any offers where the product could not be found
