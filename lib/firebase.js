...
import { getFirestore, collection, getDocs, doc, getDoc, query, where, addDoc, serverTimestamp, onSnapshot } from 'firebase/firestore';
...
// Function to fetch free delivery settings
export async function getFreeDeliverySettings() {
  try {
    ...
    if (settingsSnap.exists()) {
      ...
      // Handle both number and string types (e.g., "1+" or 1)
      if (typeof thresholdValue === 'number') {
        threshold = thresholdValue;
      } else if (typeof thresholdValue === 'string') {
        // Parse the integer from the string, ignoring non-numeric characters
      ...
      // Default to no free delivery if the document doesn't exist.
      return { threshold: 0 };
    }
  } catch (error) {
    console.error("Error fetching free delivery settings:", error);
    // Default to no free delivery on error.
    return { threshold: 0 };
  }
}

// Function to fetch dynamic delivery charges
export async function getDeliveryCharges() {
  ...
  try {
    ...
    if (settingsSnap.exists()) {
      const data = settingsSnap.data();
      const insideCharge = typeof data.inside === 'number' ? data.inside : defaults.inside;
      const outsideCharge = typeof data.outside === 'number' ? data.outside : defaults.outside;
      return { inside: insideCharge, outside: outsideCharge };
    } else {
      console.warn("Delivery charge settings not found in Firestore. Using default values.");
      return defaults;
    }
  } catch (error) {
...
// Function to fetch quantity based coupons
...
// Function to subscribe to maintenance mode changes
export function subscribeToMaintenanceMode(callback) {
  ...
  return onSnapshot(maintenanceRef, (docSnapshot) => {
    if (docSnapshot.exists()) {
      callback(docSnapshot.data().enabled === true);
    } else {
      callback(false);
    }
  }, (error) => {
...
export async function submitContactForm(formData) {
  try {
    ...
    await addDoc(messagesCol, {
      ...formData,
      createdAt: serverTimestamp(),
      status: 'new', // Add a status for easier tracking
    });
  } catch (error) {
